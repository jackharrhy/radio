...
describe('RadioClient playback UI behavior', () => {
  it('starts a different selected track from 0 instead of reusing current track position', async () => {
    ...
    socket.emit('message', message('SCHEDULED_PLAY', { trackId: 'track-1', trackTimeSeconds: 42, serverTimeToExecute: performance.timeOrigin + performance.now() }))
    await flush()
    socket.sent = []

    client.play('track-2')

    assert.partialDeepEqual(socket.sent.at(-1), { type: 'PLAY', trackId: 'track-2', trackTimeSeconds: 0 })
    client.dispose()
  })
  ...
  it('does not mark a paused track as ended when Web Audio fires ended after stop', async () => {
    ...
    socket.emit('message', message('SCHEDULED_PLAY', { trackId: 'track-1', trackTimeSeconds: 30, serverTimeToExecute: performance.timeOrigin + performance.now() }))
    await flush()
    socket.emit('message', message('SCHEDULED_PAUSE', { trackId: 'track-1', trackTimeSeconds: 12, serverTimeToExecute: performance.timeOrigin + performance.now() }))
