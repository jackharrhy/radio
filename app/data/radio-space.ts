...
export class RadioSpace {
  ...
  processNtp(
    ...
    if (data.clientRTT !== undefined && data.clientRTT > 0) {
      ...
      client.info.rtt =
        client.info.rtt > 0
          ? client.info.rtt * (1 - alpha) + data.clientRTT * alpha
          : data.clientRTT;
    }
  ...
  private getScheduledExecutionTime(): number {
    ...
    for (let client of this.clients.values()) {
      ...
      if (client.info.compensationMs > maxCompensation)
        maxCompensation = client.info.compensationMs;
    }
    ...
    this.heartbeat = setInterval(() => {
      ...
      for (let [clientId, client] of this.clients.entries()) {
        ...
        if (silentMs > (this.options.livenessReapAfterMs ?? DEFAULT_LIVENESS_REAP_AFTER_MS)) {
          ...
          silentMs > (this.options.livenessPingAfterMs ?? DEFAULT_LIVENESS_PING_AFTER_MS)
        ) {
